...
const Navigation = ({ title, options, onOptionSelected }: Props) => {
  ...
  const baseClass =
    "cursor-pointer flex flex-row items-center py-1 px-4 rounded-full";
  return (
        ...
        <ul className="border-l-2 border-vector">
          {options.map((option) => (
            <div className="flex items-center mb-2" key={option.key}>
              <hr className="border-l-2 border-vector w-3 mr-2" />
              <li
                className={`${baseClass} ${option.selected && "bg-vector"}`}
